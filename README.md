...
для расстановки пунктуации и чистки от слов-паразитов, присылает готовую
расшифровку PDF-файлом. Для сообщений длиннее `DIARIZATION_MIN_SECONDS`
(по умолчанию 180 сек) дополнительно разделяет речь по спикерам
(pyannote.audio).
...
4. Если длительность ≥ `DIARIZATION_MIN_SECONDS` — аудио конвертируется в
   WAV (ffmpeg) и `pyannote.audio` определяет, где говорит какой спикер;
   сегменты размечаются как «Спикер 1: ...», «Спикер 2: ...».
...
6. Готовый текст оформляется в PDF и отправляется файлом.

Если диаризация даёт сбой — бот отправит текст без разметки спикеров,
не прерывая обработку. Если не получилось собрать PDF — как исключение
отправляется обычным текстовым сообщением, чтобы результат не потерялся.
